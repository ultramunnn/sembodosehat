...
function tampilkanKontenTerbaru($conn, $limit = 5)
{
    $query = "SELECT k.id, k.judul, k.deskripsi, k.gambar, k.video_link, k.tipe_konten,
        rp.nama AS penyakit_nama FROM konten k JOIN konten_penyakit kp ON k.id = kp.konten_id
        JOIN riwayat_penyakit rp ON kp.penyakit_id = rp.id ORDER BY k.created_at DESC
        LIMIT $limit
    ";
...
function tampilkanArtikelTerbaru($conn, $limit = 6, $offset = 0)
{
  ...
  FROM konten k JOIN konten_penyakit kp ON k.id = kp.konten_id JOIN riwayat_penyakit rp ON kp.penyakit_id = rp.id WHERE k.tipe_konten = 'Artikel' ORDER BY k.created_at DESC
...
function tampilkanVideoTerbaru($conn, $limit, $offset)
{
    ...
    $sql = "SELECT k.id, k.judul, k.deskripsi, k.video_link, k.tipe_konten, rp.nama AS penyakit_nama FROM konten k JOIN konten_penyakit kp ON k.id = kp.konten_id JOIN riwayat_penyakit rp ON kp.penyakit_id = rp.id WHERE k.tipe_konten = 'Video' ORDER BY k.created_at DESC LIMIT $limit OFFSET $offset";
