...
<section class="w-full max-w-full mx-auto min-h-[600px] bg-transparent px-4 sm:px-6 py-2">
    ...
    <div class="flex gap-4 sm:gap-6 overflow-x-auto hide-scrollbar px-2 py-2">
        ...
        <?php else: ?>
            ...
            <?php foreach ($kontens as $konten): ?>
                ...
                <article
                    class="max-w-[300px] sm:max-w-[500px] h-full rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300 flex-shrink-0">

                    <?php if ($konten['tipe_konten'] === 'Artikel'): ?>
                        <img src="<?= $konten['gambar'] ? '../assets/img/artikel/' . $konten['gambar'] ?? '' : 'default.jpg' ?>"
                            alt="<?= $konten['judul'] ?? '' ?>" class="w-full h-[150px] sm:h-[250px] rounded-t-lg object-cover" />
                    ...
                    <?php elseif ($konten['tipe_konten'] === 'Video'): ?>
                        ...
                        <?php if ($video_id): ?>
                            <img src="https://img.youtube.com/vi/<?= $video_id ?? '-' ?>/hqdefault.jpg" alt="Thumbnail"
                                class="w-full h-[150px] sm:h-[350px] rounded-t-lg object-cover" />
                    ...
                    <div class="bg-green-950/95 rounded-b-lg p-4 sm:p-6">
                        <h3 class="text-white text-lg sm:text-xl font-semibold font-poppins mb-2 sm:mb-4">
                            <?= $konten['judul'] ?? '-' ?>
                        </h3>
                        <p class="text-white text-sm sm:text-l font-light font-poppins mb-4 sm:mb-6 leading-relaxed">
                            <?= $konten['deskripsi'] ?? '-' ?>
                        </p>
                        <div class="border border-white mb-2 sm:mb-4"></div>
                        <div class="flex items-center gap-2 ">
                            <span
                                class="bg-green-500/50 rounded px-2 py-1 text-white text-xs sm:text-sm font-light font-poppins">
                                <?= $konten['tipe_konten'] ?? '-' ?>
                            </span>
                            <span
                                class="bg-green-500/50 rounded px-2 py-1 text-white text-xs sm:text-sm font-light font-poppins">
